...
class AnalysisTest(unittest.TestCase):
    def test_can_deline_tokens(self):
        self.assertEqual(list(analysis._delined_tokens([(token.Comment, "# a")])), [(token.Comment, "# a")])
        self.assertEqual(
    ...
    def test_can_compute_python_line_parts(self):
        ...
        self.assertEqual(list(analysis._line_parts(python_lexer, "#")), [set("d")])
        self.assertEqual(list(analysis._line_parts(python_lexer, "s = 'x'  # x")), [set("cds")])

    def test_can_detect_white_text(self):
        python_lexer = lexers.get_lexer_by_name("python")
        self.assertEqual(list(analysis._line_parts(python_lexer, "{[()]};")), [set()])
        self.assertEqual(list(analysis._line_parts(python_lexer, "pass")), [set()])
    ...
    def test_has_no_duplicate_in_pygount_source(self):
        ...
        for source_path in source_paths:
            if source_path.endswith(".py"):
                ...
                self.assertIsNone(
                    duplicate_path, "{0} must not be duplicate of {1}".format(source_path, duplicate_path)
                )
